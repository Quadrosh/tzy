...
<!--    <h1>--><?//= Html::encode($this->title) ?><!--</h1>-->
    <p>
        <?= Html::a('Звонок', ['/preorders/create-call'], ['class' => 'btn btn-success']) ?>
    </p>

    <div class="row">
        <?php $form = ActiveForm::begin([
            'id'=>'filterForm',
            'action' => ['/preorders/lead-quality'],
            'method' => 'post',
        ]); $filterForm = new common\models\FilterForm(); ?>

        <div class="col-sm-4  text-center">
            <?= $form->field($filterForm, 'manager')
                ->dropDownList(\yii\helpers\ArrayHelper::map(common\models\Manager::find()->orderBy('name')->all(), 'id','name'),[
...
//                    'class'=>'selectpicker',
        ...
        <div class="col-sm-4  text-center">
            <?= $form->field($filterForm, 'status')
                ->dropDownList(\yii\helpers\ArrayHelper::map(common\models\OrderStatus::find()->orderBy('name')->all(), 'id','name'),[
...
//            'manager',
            [
                ...
                'value' => function($data)
                {
                    $theData = common\models\Manager::find()->where(['id'=>$data['manager']])->one();
                    return $theData['name'];
                },
            ],
//            'quality',
